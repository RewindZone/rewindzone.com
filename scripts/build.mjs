...
write('index.html', layout({
  ...
  body: `<main id="main" class="shell">
    ...
    <section class="featured-categories"><div class="section-heading"><h2>Browse the archive</h2><a href="/categories/">All categories</a></div><div class="category-grid">${categories.filter(category => category.group === 'type').map(category => `<a class="category-card" href="/categories/${category.group}/${category.slug}/"><span>${escapeHtml(category.label)}</span><strong>${category.articles.length} articles</strong></a>`).join('')}</div></section>
    <section class="home-latest"><div class="section-heading"><h2>Recently added to the archive</h2><a href="/archive/">Full archive</a></div><p>A selection from the collection. Articles retain their original publication dates; streaming availability may have changed.</p><ol class="article-list">${sortedArticles.slice(0, 12).map(articleCard).join('\n')}</ol></section>
  </main>`,
}))
